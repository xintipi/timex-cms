<template>
  <table class="table-form">
    <thead>
      <tr>
        <th>{{ $t('user.going_out_setting') }}</th>
        <th>
          ...
          >
            <a-collapse-panel key="1">
              <table class="table-body">
                <tbody>
                  <tr class="table__row">
                    <td>{{ $t('user.outing_time_rounding') }}</td>
                    <a-row>
                      <a-col :lg="12" :xl="12" style="padding-right: 10px">
                        <Field
                          v-slot="{ value, handleChange }"
                          v-model="form.outing_rounding"
                          name="outing_rounding"
                      ...
                      <a-col :lg="12" :xl="12">
                        <Field
                          v-slot="{ value, handleChange }"
                          v-model="form.outing_rounding_time"
                          name="outing_rounding_time"
                          style="display: inline-block; width: 100%"
                          as="td"
                        >
                          <a-form-item>
...
<script lang="ts">
  ...
  const initState = {
    outing_rounding: null,
    outing_rounding_time: null,
  };

  export default defineComponent({
    name: 'FormGoingOutSetting',
