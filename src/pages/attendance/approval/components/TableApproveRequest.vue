<template>
  <div class="approve-request">
    ...
    <div class="approve-request__options" ref="approveRef">
      <a-pagination
    ...
    <div class="approve-request__table">
      <a-table
        id="list-table"
        :columns="columns"
        ...
        :scroll="{ x: 1008, y: windowHeight - heightApprove.height - heightSearch - 182 }"
        size="middle"
        bordered
      >
        <template #bodyCell="{ column, record, text }">
          ...
          <template v-if="column.key === 'application_content'">
            <a-row :gutter="25">
              ...
              <a-col :span="24">
                <span style="color: green">{{ $t('attendance.applied') }}</span>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      const formState = ref<formState>({ note: '' });
      const query = ref<string>('');
      const searchRef = ref<null>(null);
      const approveRef = ref<null>(null);
      ...
      const heightApprove = reactive(useElementSize(approveRef));
