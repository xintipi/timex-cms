...
export function createStorage(storage: Storage, key: string, options?: StorageOptions) {
  ...
  const getItem = () => {
    try {
      const itemVal = tryDeserialize(storage.getItem(key), deserializer, isParsing);
